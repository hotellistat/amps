...
type Job struct {
	Created  time.Time
	JobID    string       // Only store the job ID, not the full message content
...
func (jm *Manifest) InsertJob(ID string) {
...
func (jm *Manifest) InsertJobWithDelivery(ID string, delivery AMQPDelivery) {
	messagesInserted.Inc()
