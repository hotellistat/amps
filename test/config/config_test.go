...
func TestGetEnv(t *testing.T) {
	...
	value := config.GetEnv("GO_TEST_GETENV", "default")

	if value != "getEnv" {
		t.Error("Could not get environment variable")
	}

	value2 := config.GetEnv("GO_TEST_GETENV_NA", "default")
...
func TestGetEnvRequired(t *testing.T) {
	...
	exitCalled := false
	value := config.GetEnvRequired("GO_TEST_GETENVREQUIRED", func(code int) { exitCalled = true })
	...
	config.GetEnvRequired("GO_TEST_GETENVREQUIRED_NA", func(code int) {
		exitCalled = true
	})
...
func TestGetEnvAsInt(t *testing.T) {
	...
	value := config.GetEnvAsInt("GO_TEST_GETENV", 1)

	if value != 5 {
		t.Error("Could not get environment int variable")
	}

	value2 := config.GetEnvAsInt("GO_TEST_GETENV_NA", 3)
...
func TestGetEnvAsDuration(t *testing.T) {
	...
	value := config.GetEnvAsDuration("GO_TEST_GETENV", "1m")
	...
	value2 := config.GetEnvAsDuration("GO_TEST_GETENV_NA", "3m")
	...
	value3 := config.GetEnvAsDuration("GO_TEST_GETENV_FALLBACK", "8m")
...
func TestGetEnvAsBool(t *testing.T) {
	...
	value := config.GetEnvAsBool("GO_TEST_GETENV", true)

	if value != false {
		t.Error("Could not get environment bool variable")
	}

	value2 := config.GetEnvAsBool("GO_TEST_GETENV_NA", true)
